...
export const Goals: React.FC = () => {
  ...
  const filteredKpis = useMemo(() => {
    ...
    if (!term) {
      return orgGoalsWithKPIs;
    }

    return orgGoalsWithKPIs.filter((goal) => {
      const goalFields = [goal.title, goal.description, goal.level]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
      if (goalFields.includes(term)) {
        return true;
      }
      const kpis = (goal.metadata?.kpis || []) as KPI[];
      return kpis.some((kpi) =>
        [kpi.name, kpi.description, kpi.target, kpi.unit, kpi.frequency]
          .filter(Boolean)
          .join(' ')
          .toLowerCase()
          .includes(term)
  ...
  const renderKpiTab = () => (
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Organizational KPIs</h2>
          <p className="text-gray-600 text-sm">
            Search KPIs generated for strategic and cascading goals.
          </p>
        </div>
        <input
          type="text"
          ...
          placeholder="Search KPI or goal..."
      ...
      {orgKpiLoading ? (
        ...
        <div className="card text-center py-8">
          <p className="text-gray-600">
            No KPIs available. Generate KPIs from the Organizational Goals workspace to see them here.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {filteredKpis.map((goal) => (
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Track personal goals and reference organizational KPIs.
          </p>
        </div>
      ...
      {activeTab === 'goals' ? renderGoalsTab() : renderKpiTab()}
